...
class professorDAO
{
    public function save($professor)
    {
        ...
        try {
            if ($professor->getId() != "") {
                $statement = $pdo->prepare("UPDATE `sga`.`professor`
                                                        SET
                                                        `Nome` = :nome,
                                                        `Cargo` = :cargo 
                ...
                $statement = $pdo->prepare("INSERT INTO `sga`.`professor`
                                                            (`Nome`,
                                                             `Cargo`)
                                                        VALUES
                                                            (:nome,
            ...
            if ($statement->execute()) {
                ...
                throw new PDOException("Erro: Não foi possível executar a declaração sql Save");
            }
    ...
    public function atualizar($professor)
    {
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT `professor`.`idProfessor`,
                                                         `professor`.`Nome`,
                                                         `professor`.`Cargo`
                                                    FROM `sga`.`professor`
                                                    WHERE `idProfessor` = :id ;");
            ...
            if ($statement->execute()) {
                ...
                throw new PDOException("Erro: Não foi possível executar a declaração sql Update");
            }
        } catch (PDOException $erro) {
            return "Erro: " . $erro->getMessage();
        }
    ...
    public function tabelapaginada()
    {
        ...
        $sql = "SELECT `professor`.`idProfessor`,
                        `professor`.`Nome`,
                        `professor`.`Cargo`
                 FROM `sga`.`professor` LIMIT {$linha_inicial}, " . QTDE_REGISTROS;
